...
if [ $# -lt 1 ]; then
  echo "Usage: conv2linux.sh [FILENAME]..."
  exit 1
fi
...
errorfile=conv2linux.$(date +%Y.%m.%d).log
...
for file in $*; do
  ...
  # Delete the tail of line.
  echo -en "\033[01;33m"$filename"\033[00m: delete <CR>... "
  sed 's/.$//g' $tempfile > $convfile 2>> $temperrorfile
  echo -e "\033[01;32mdone"
  echo -en "\033[00m"
