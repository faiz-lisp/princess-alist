...
function initSearch()
{
  ...
  _old_input_field_value = _inputField.value;
  _select_index = -1;
  _host = "";
...
var mover = function()
{
  ...
  /*
   * Cancel the selection by keybord.
   */
  if (_select_index != -1)
  {
...
var mdown_2 = function()
{
  ...
  window.location = _host + "type.fcg?type=" + type + "&q=" + _inputField.value;
};
...
function keyEvent(event)
{
  ...
  else if (code == 13)
  {
    if (_select_index > line_pos)
    {
      ...
      window.location = _host + "type.fcg?type=" + type + "&q=" + _old_input_field_value;
    }
  }
